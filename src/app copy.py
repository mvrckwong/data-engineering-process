...
from config_paths import LOGS_PATH, DATA_PATH
...
def calculate_summary_statistics(user_data:pd.DataFrame) -> Tuple[int, int, list]:
    ...
    average_hours_per_project = user_data.groupby('project')['hours'].mean().to_dict()
...
def main() -> None:
    ...
    try:
        ...
        df = load_data(DATA_PATH / 'raw' / f'{CONFIGS.CURRENT_TABLE}.csv')
        logger.info('Got data from the raw data directory.')
    
    # Get the list of users based on pandas data
    users:List[str] = [''] + df['user'].unique().tolist()
